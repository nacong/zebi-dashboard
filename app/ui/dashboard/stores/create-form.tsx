...
import { createStore } from '@/app/lib/actions';
import { Category, CATEGORY_LIST, StoreCreate, StoreState } from '@/app/lib/definitions';
...
const DEFAULT_MARKER: Marker = { position: MAP_CENTER };
const DEFAULT_STORE: StoreCreate = {
  name: '',
  category: '한식',
  url: '',
  lat: MAP_CENTER.lat,
...
export default function StoreCreateForm() {
  const [store, setStore] = useState<StoreCreate>(DEFAULT_STORE);
  const [selectedPlace, setSelectedPlace] = useState<Marker>(DEFAULT_MARKER);

  const [errors, setErrors] = useState<StoreState['errors']>({});
  const [message, setMessage] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  const handleChange = <T extends keyof StoreCreate>(
    key: T,
    value: StoreCreate[T]
  ) => {
    setStore((prev) => ({
  ...
  // (검색해도 안나오는 가게면 수동으로 맵을 클릭해요.)
  // 맵을 클릭했을 때 가게 정보의 lat, lon을 바꿔요.
